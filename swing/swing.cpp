...
// Kept just because I cannot bear to erase it

/**
 * @function swingVerticalStep
 * @brief Start and end vertical movement + cycloid on top + linear change in yaw
 */
/*
void swingVerticalStep( double _x0, double _y0, double _theta0,
	    double _x1, double _y1, double _theta1,
	    int _count, 
	    bool _isLeft,
	    double _pos[N][3],
  ...
  double h_perc = 0.1; // % of max z displacement (to make h dependent of size hMax)
  int m = _count*(h_perc / 2.0); // steps for vertical movement (up and down)
  ...
  dtheta = (_theta1 - _theta0) / (cycloid_count - 1);
  dt = (t1 - t0) /(cycloid_count - 1);

  // Generate the values for the cycloid
  for( int i = 0; i < cycloid_count; ++i ) {
    t = t0 + dt*i;
    xp_t = r*(t-sin(t));
    yp_t = r*(1-cos(t));
    
    _pos[i+m][0] = _x0 + xp_t*cos_a;
    _pos[i+m][1] = _y0 + xp_t*sin_a;
    _pos[i+m][2] = h + yp_t;
    _yaw[i+m] = _theta0 + dtheta*i;
  ...
  // m: number of steps 
  getSpline3( m-1, 0, h, 0, 0, 
	      a0, a1, a2, a3 );

  for( int i = 0; i < m; ++i ) {
    _pos[i][0] = _x0;
    _pos[i][1] = _y0;
    _pos[i][2] = a3*pow(i,3) + a2*pow(i,2) + a1*i + a0;
    _yaw[i] = _theta0;
  }

  getSpline3( m-1, h, 0, 0, 0, 
	      a0, a1, a2, a3 );

  for( int i = 0; i < m; ++i ) {
    _pos[cycloid_count + m + i][0] = _x1;
    _pos[cycloid_count + m + i][1] = _y1;
    _pos[cycloid_count + m + i][2] = a3*pow(i,3) + a2*pow(i,2) + a1*i + a0;
    _yaw[cycloid_count + m + i] = _theta1;
  }
...
/**
 * @function swingVerticalStepSplineYaw
 * @brief As swingVerticalStep but spline for yaw instead of same change rate
 */
/*
void swingVerticalStepSplineYaw( double _x0, double _y0, double _theta0,
	    double _x1, double _y1, double _theta1,
	    int _count, 
	    bool _isLeft,
	    double _pos[N][3],
  ...
  getSpline3( cycloid_count - 1, _theta0, _theta1, 0, 0,
	      a0, a1, a2, a3 );
  ...
  for( int i = 0; i < cycloid_count; ++i ) {
    ...
    _pos[i+m][1] = _y0 + xp_t*sin_a;
    _pos[i+m][2] = h + yp_t;
    _yaw[i+m] =  a3*pow(i,3) + a2*pow(i,2) + a1*i + a0;
